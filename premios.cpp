...
//  if(premio13>recaudacion*0.08) premio13=recaudacion*0.08;
  ...
  if(premio10>=2500) premio10*=0.8;
  if(premio11>=2500) premio11*=0.8;
  if(premio12>=2500) premio12*=0.8;
  if(premio13>=2500) premio13*=0.8;
  if(premio14>=2500) premio14*=0.8;
...
void todas(){
  ...
  fout.open("PREMIOS.txt");
  for(int i=n_col-1; i>=0; i--){
    int cssize;
    char cschar;
    cssize=colarchivo[i].size()-1;
    cschar=colarchivo[i][cssize];
    if(cschar!='1' && cschar!='X' && cschar!='2') colarchivo[i][cssize]=' ';
